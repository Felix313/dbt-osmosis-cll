...
from dbt_osmosis_cll.osmosis_propagation.transforms import annotate_column_origins
...
def patched(*, results=None, settings=None, origin=None, origin_desc=None, col_docs=None):
    ...
    *settings* maps a setting key to its value for _get_setting_for_node (only the
    ...
    stack.enter_context(
        mock.patch("dbt_osmosis_cll.integration.cll.get_column_origin", lambda *a, **k: origin)
    )
    stack.enter_context(
        mock.patch(
