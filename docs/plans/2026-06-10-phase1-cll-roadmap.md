...
| 5 | Multi-source origins end-to-end: `progenitors: list[(model, col)]` on `ColumnLineageResult` (generalize `union_branches`); annotate "Computed here from A.X, B.Y" | computed endpoint columns don't say what feeds them | `api.py`, parser (depends #4), `annotations.py`, `transforms.py` annotate pass | M | done 2026-06-11 (also fixed: union_branches was missing from the disk-cache field list — warm-cache runs silently lost union agreement data) |
| 6 | Ship visualizer: `dbt-osmosis-cll lineage explore` wiring existing HTML explorer to `ManifestCatalogReader` (manifest-only, no catalog.json/warehouse; deps behind `lineage-ui` extra) | goal-4 visualization not in product | `cli/main.py`, `lineage/service.py` | M | done 2026-06-11 — note: the explorer does its own one-shot registry parse; the CLL disk cache (`target/cll_cache.json`) belongs to the propagation pipeline and is intentionally not shared |
| 7 | Machine-readable origins by default on endpoint/DP layers (`write-cll-tags-to-meta: true` + `annotate: always` pattern, docs) | catalog tools don't see origins | `docs/USAGE.md` (config.py untouched: per-node resolution already existed; global default deliberately stays `false` to avoid diff churn — the endpoint-layer pattern is the documented recommendation) | S | done 2026-06-11 |
| 8 | Endpoint trust report: extend `doc_health` with per-model authored / inherited(origin) / glossary / annotation-only / CLL-failed breakdown, JSON for CI | "documented" ≠ "trustworthy"; CI gate vs silent CLL regressions | `commands/doc_health.py` (JSON keys additive; CLI unchanged) | S/M | done 2026-06-11 |
