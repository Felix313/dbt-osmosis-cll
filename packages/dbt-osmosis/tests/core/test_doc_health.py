...
def patched(nodes, *, cll_failures=None):
    ...
    stack.enter_context(mock.patch("dbt_osmosis.core.cll.get_cll_results", lambda _c, _n: []))
    stack.enter_context(
        mock.patch("dbt_osmosis.core.cll.get_cll_failures", lambda _c: frozenset(cll_failures))
    )
    with stack:
        yield
...
def test_check_cll_collects_failures_and_skips_sources():
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.enter_context(mock.patch("dbt_osmosis.core.cll.get_cll_results", cll_results))
        stack.enter_context(
            mock.patch("dbt_osmosis.core.cll.get_cll_failures", lambda _c: frozenset({"m"}))
        )
