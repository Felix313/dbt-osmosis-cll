...
from dbt_osmosis_cll.osmosis_propagation.exceptions import MissingOsmosisConfig, PathResolutionError
...
def get_target_yaml_path(context: YamlRefactorContextProtocol, node: ResultNode) -> Path:
    ...
    if not tpl:
        if node.resource_type == NodeType.Source:
            ...
            logger.debug(
                "No source_definitions entry for => %s; using original_file_path", node.unique_id
            )
        else:
...
def create_missing_source_yamls(context: t.Any) -> None:
    ...
    from dbt_osmosis_cll.osmosis_propagation.introspection import _find_first, get_columns
    from dbt_osmosis_cll.osmosis_propagation.schema.reader import (
        _read_yaml,
    )
    from dbt_osmosis_cll.osmosis_propagation.schema.writer import _write_yaml
