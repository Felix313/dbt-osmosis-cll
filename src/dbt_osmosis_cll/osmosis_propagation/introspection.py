...
class ProjectVarsSource(ConfigurationSource):
    ...
    - dbt_osmosis_cll.<key> (under dbt_osmosis_cll top-level key)
    ...
    def get(self, key: str) -> t.Any | None:
        ...
        # Check dbt_osmosis_cll top-level key (snake_case variant)
        dbt_osmosis_vars_snake = vars_dict.get("dbt_osmosis_cll", {})
...
class SettingsResolver:
    ...
    def get_yaml_path_template(
        ...
        `dbt_osmosis_cll` key (without a setting suffix) in config sources.

        Precedence (highest to lowest):
            1. node.config.extra["dbt-osmosis"] or ["dbt_osmosis_cll"]
            2. node.config.meta["dbt-osmosis"] or ["dbt_osmosis_cll"] (dbt 1.10+)
            3. node.meta["dbt-osmosis"] or ["dbt_osmosis_cll"]
            4. node.unrendered_config["dbt-osmosis"] or ["dbt_osmosis_cll"] (dbt 1.10+)
        ...
        keys = ("dbt-osmosis", "dbt_osmosis_cll")
...
def get_columns(
    ...
    if context.project.config.disable_introspection:
        logger.warning(
            ":warning: Introspection is disabled, cannot introspect columns => %s",
            rendered_relation,
        )
        return normalized_columns
    ...
    # Try get_catalog_by_relations first: unlike get_columns_in_relation (DESCRIBE TABLE),
    # it queries INFORMATION_SCHEMA.COLUMNS which includes column comments.
    # This is a targeted single-relation query — not a full catalog build.
    ...
    try:
        ...
        if catalog_table and len(catalog_table.rows) > 0:
            logger.info(
                ":books: Fetched columns with comments via catalog_by_relations for => %s",
                rendered_relation,
            )
            # Build ColumnMetadata from catalog rows (includes comment field)
            catalog_columns: list[ColumnMetadata] = []
            ...
            if catalog_columns:
                with _COLUMN_LIST_CACHE_LOCK:
                    _COLUMN_LIST_CACHE[cache_key] = tuple(catalog_columns)
...
class PropertyAccessor:
    ...
    def _get_from_yaml(
        ...
        """
        from dbt_osmosis_cll.osmosis_propagation.inheritance import _get_node_yaml
