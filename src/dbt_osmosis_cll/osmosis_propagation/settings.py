...
def _create_yaml_instance() -> ruamel.yaml.YAML:
    from dbt_osmosis_cll.osmosis_propagation.schema.parser import create_yaml_instance
...
class YamlRefactorSettings:
    ...
    introspect_sources_only: bool = True
    """When True (default), DB introspection is restricted to source nodes only.
    Model columns are always resolved via CLL from compiled SQL — never from the warehouse.
    Sources have no SQL definition so DB is always the correct source of truth for them.
    Set to False to allow DB fallback for model nodes when CLL cannot resolve columns."""
    compile_on_cll_failure: bool = True
    """When True (default), automatically run 'dbt compile --select <model>' when CLL
    fails because the compiled SQL file is missing from target/compiled/.
    After compile, CLL is retried before falling back."""
    db_fallback_on_cll_failure: bool = False
    """When True, fall back to DB introspection for model nodes if CLL still returns
    no columns after a compile attempt (or compile_on_cll_failure=False).
    When False (default), osmosis skips column injection for that model and logs an error.
    Only relevant when introspect_sources_only=True."""
...
class YamlRefactorContext:
    ...
    def source_definitions(self) -> dict[str, t.Any]:
        ...
        toplevel_conf = self._find_first(
            [c.get(k, {}) for k in ["dbt-osmosis", "dbt_osmosis_cll"]],
            lambda v: bool(v),
            {},
        )
