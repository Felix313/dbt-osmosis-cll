...
def yaml_opts(func: t.Callable[P, T]) -> t.Callable[P, T]:
    ...
    @click.option(
        ...
        default=None,
        help="When enabled (default), DB introspection is restricted to source nodes only. "
        "Model columns are resolved from CLL/compiled SQL. "
        "Use --no-introspect-sources-only to allow DB fallback for model nodes when CLL returns nothing.",
    )
    @click.option(
        "--compile-on-cll-failure/--no-compile-on-cll-failure",
        default=None,
        help="When enabled (default), automatically run 'dbt compile --select <model>' "
        "if compiled SQL is missing and CLL cannot resolve columns. CLL is retried after compile.",
    )
    @click.option(
        "--db-fallback-on-cll-failure/--no-db-fallback-on-cll-failure",
        default=None,
        help="When enabled, fall back to DB introspection for model nodes if CLL still returns "
        "nothing after a compile attempt. Default is disabled — osmosis errors instead.",
    )
    @click.option(
...
def _run_formatter_if_configured(context: YamlRefactorContext) -> None:
    ...
    if formatter and not context.settings.dry_run and context.written_files:
        from dbt_osmosis_cll.osmosis_propagation.formatting import run_external_formatter
...
def staging(
    ...
    **kwargs: t.Any,
    ...
    This command generates deterministic staging models from source tables
    via dbt-core-interface.
    """
    logger.debug("osmosis command invoked")
    settings = DbtConfiguration(
        project_dir=t.cast(str, project_dir),
...
def _output_diff_text(results: dict[str, t.Any], severity_filter: str) -> None:
    ...
    for result in results.values():
        ...
        if severity_filter != "all":
            from dbt_osmosis_cll.osmosis_propagation.commands.diff import ChangeSeverity
        ...
        from dbt_osmosis_cll.osmosis_propagation.commands.diff import ColumnRenamed
...
def suggest(
    ...
    """Suggest dbt tests for models based on patterns learned from the project.

    \f
    This command analyzes your dbt project's existing test conventions and suggests
    appropriate tests for each model.

    Examples:
        dbt-osmosis-cll test suggest
        dbt-osmosis-cll test suggest --fqn my_project.my_model
        dbt-osmosis-cll test suggest --format json
    ...
    if fqn or models:
        ...
        try:
            ...
            results = suggest_tests_for_project(context=context)
        except Exception as e:
